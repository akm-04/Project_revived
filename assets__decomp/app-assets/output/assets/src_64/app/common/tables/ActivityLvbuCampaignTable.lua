local var_0_0 = class("ActivityLvbuCampaignTable")

function var_0_0.ctor(arg_1_0)
	arg_1_0.type_ = {}
	arg_1_0.eventId_ = {}
	arg_1_0.x_ = {}
	arg_1_0.y_ = {}
	arg_1_0.icon_ = {}

	import("app.common.tables.TableParser").parse("activity_lvbu_campaign.lua", function(arg_2_0)
		local var_2_0 = tonumber(arg_2_0.id)

		arg_1_0.type_[var_2_0] = tonumber(arg_2_0.type)
		arg_1_0.eventId_[var_2_0] = tonumber(arg_2_0.event_id)
		arg_1_0.x_[var_2_0] = tonumber(arg_2_0.x)
		arg_1_0.y_[var_2_0] = tonumber(arg_2_0.y)
		arg_1_0.icon_[var_2_0] = arg_2_0.icon
	end)
end

function var_0_0.type(arg_3_0, arg_3_1)
	return arg_3_0.type_[arg_3_1] or 0
end

function var_0_0.eventId(arg_4_0, arg_4_1)
	return arg_4_0.eventId_[arg_4_1] or 0
end

function var_0_0.x(arg_5_0, arg_5_1)
	return arg_5_0.x_[arg_5_1] or 0
end

function var_0_0.y(arg_6_0, arg_6_1)
	return arg_6_0.y_[arg_6_1] or 0
end

function var_0_0.icon(arg_7_0, arg_7_1)
	return arg_7_0.icon_[arg_7_1] or 0
end

function var_0_0.getCounts(arg_8_0)
	return #arg_8_0.x_
end

return var_0_0
