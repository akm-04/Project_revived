local var_0_0 = class("VipFreeGiftTable")

function var_0_0.ctor(arg_1_0)
	arg_1_0.name_ = {}
	arg_1_0.vip_ = {}
	arg_1_0.gift_ = {}
	arg_1_0.desc_ = {}

	import("app.common.tables.TableParser").parse("vip_free_gift.lua", function(arg_2_0)
		local var_2_0 = tonumber(arg_2_0.id)

		arg_1_0.name_[var_2_0] = arg_2_0.name
		arg_1_0.vip_[var_2_0] = tonumber(arg_2_0.vip)
		arg_1_0.gift_[var_2_0] = tonumber(arg_2_0.gift)
		arg_1_0.desc_[var_2_0] = arg_2_0.description
	end)
end

function var_0_0.name(arg_3_0, arg_3_1)
	return arg_3_0.name_[arg_3_1] or ""
end

function var_0_0.vip(arg_4_0, arg_4_1)
	return arg_4_0.vip_[arg_4_1] or 0
end

function var_0_0.gift(arg_5_0, arg_5_1)
	return arg_5_0.gift_[arg_5_1] or 0
end

function var_0_0.desc(arg_6_0, arg_6_1)
	return arg_6_0.desc_[arg_6_1] or ""
end

function var_0_0.gifts(arg_7_0)
	return arg_7_0.gift_ or {}
end

return var_0_0
