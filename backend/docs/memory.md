...
## Stage 1 modular backend coding pass — 2026-08-16

Static-analysis waterfall has been stopped by user direction. Pass 19 is the baseline documentation set for implementation. The old flat `server.py + game_logic.py` backend is now treated as a reference for confirmed cookie/payload behavior, not as the final architecture.

A new modular backend has been created under `gxb_backend/` with:

- `transport/` for SDK routes, engine routes, payload decoding, response building;
- `protocol/` for the MID catalog and transport classification;
- `state/` for canonical account/player state;
- `handlers/` for domain-owned MID handlers;
- `dispatch/engine_dispatcher.py` for MID-to-handler routing;
- `chat/stub.py` for a minimal TCP acceptor advertised by MID 192;
- root `server.py` and `run.py` compatibility launchers.

Implemented Stage 1 flow targets:

`SDK login -> center discovery -> version check -> RETRIEVE_TOKEN -> detail hydration -> xydSelectServer -> MessageManager -> MainScene first-entry windows`.

Stage 1 semantic handler families currently implemented:

- SDK identity/cookie/session responses and upload/payment compatibility;
- center discovery/version/user-regions/announce/server-time/player-group;
- RETRIEVE_TOKEN and canonical bootstrap detail;
- player info, heroes, backpack, library/background, summon, illusion, friends, chat-room discovery;
- activities/board/single-activity/reward/sign/charge, arena records, task load/reward;
- status-only acknowledgement for selected early/fanout/result commands;
- compatibility fallback for unknown MIDs with logging.

Important implementation choices:

- `RETRIEVE_TOKEN.detail` now includes source-confirmed first-entry state: `17`, `49`, `81`, `836`, plus conservative safe `56`, `176`, `229`, and `2560`.
- Everything serializes from one `PlayerState` where possible.
- `GET_PLAYER_GROUP_BY_KEY` remains a scalar `"A"`, not a normal engine envelope payload.
- `LOAD_CHAT_ROOM_INFO` remains HTTP discovery; chat-bit-range MIDs are not promoted to normal Flask HTTP endpoints.
- `ChatStubServer` accepts TCP connections but does not implement chat-room semantics.
- Unknown MIDs are acknowledged for discovery/progress and logged; this is not semantic completeness.

Only simple syntax compilation was run. No Flask smoke tests, no APK testing, and no runtime gameplay claims were made.
